...
export const LeaderTodoResultModal: React.FC<Props> = ({ todo, onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl border border-indigo-100 max-w-lg w-full overflow-hidden animate-fade-in">
        {/* ヘッダー */}
        <div className="bg-gradient-to-r from-indigo-800 to-indigo-950 text-white px-6 py-4 flex items-center justify-between shadow-md">
          <div className="flex items-center gap-2">
            <span className="text-xl">✍️</span>
            <div>
              <h2 className="font-black text-lg leading-tight">TODO対応結果 ＆ 方向性記録</h2>
              <p className="text-[11px] text-indigo-200">対応完了ステートへの更新・結果方針の登録</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          <div>
            ...
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value as LeaderTodo['status'])}
              className="w-full bg-gray-50 border border-gray-300 rounded-xl p-2.5 text-xs font-bold text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-xs"
            >
              <option value="completed">🟢 実施完了（対応・記録完了で右画面へ移動）</option>
              <option value="in_progress">🔵 進行中（現在対応・調整中）</option>
              <option value="pending">🟣 要保留・確認（指示・結果待ち）</option>
              <option value="untouched">⚪️ 未実施（対応着手前）</option>
            </select>
          </div>
